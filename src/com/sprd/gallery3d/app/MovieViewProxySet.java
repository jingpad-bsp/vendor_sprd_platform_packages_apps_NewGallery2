...
public class MovieViewProxySet extends PreferenceActivity {
    ...
    public static int RTP_RTCP_MAX_PORT_VALUE = 65535;
    // min port value by default
    public static int RTP_RTCP_MIN_PORT_VALUE = 1024;
    ...
    private boolean isVodafone = false;
    ...
    BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(
                ...
                if (StandardFrameworks.getInstances().getSimState(telephonyManager, CardOne) != SIM_STATE_READY
                        && StandardFrameworks.getInstances().getSimState(telephonyManager, CardTwo) != SIM_STATE_READY) {
                    if (mApnSet != null) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(LOG_TAG, "isVodafone =" + isVodafone);
        RTP_RTCP_MAX_PORT_VALUE = isVodafone ? 32000 : 65535;
        RTP_RTCP_MIN_PORT_VALUE = isVodafone ? 6970 : 1024;
        ...
        if (!mTelephonyManager.hasIccCard()
                || (StandardFrameworks.getInstances().getCallingUserId() > 0)
                || ((StandardFrameworks.getInstances().getSimState(mTelephonyManager, CardOne) != SIM_STATE_READY)
                && (StandardFrameworks.getInstances().getSimState(mTelephonyManager, CardTwo) != SIM_STATE_READY))) {
            mApnSet.setEnabled(false);
        }
        /**@}*/
        ...
        mProxyIpAdress.getEditText().addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length() >= 64) {
                    Toast.makeText(MovieViewProxySet.this, R.string.input_reached_max_length, Toast.LENGTH_SHORT).show();
                }
            }
...
//        GalleryUtils.killActivityInMultiWindow(this, GalleryUtils.DONT_SUPPORT_VIEW_VIDEO);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        if ("ApnEntrySet".equals(key)) {
            ...
            final int[] subIdList =
                    StandardFrameworks.getInstances().getActiveSubscriptionIdList(
    ...
    public void setProxyPortPreferenceChangeListener(
        ...
        ProxyTextPreference
                .setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                    ...
                    public boolean onPreferenceChange(Preference p,
                                                      Object newValue) {
                        String Proxyout = checkPortFormat((String) newValue);
    ...
    public String checkPortFormat(String value) {
        if (value == null || value.length() == 0) {
            return "";
        } else {
            int interger_value = Integer.parseInt(value);
            value = String.valueOf(interger_value);
            return value;
        }
    ...
    public void loadMovieViewContentProvider() {
        ...
        if (cursor != null) {
            try {
                ...
                ProxyEnble = proxy_enable != 0;
            } finally {
                cursor.close();
            }
        } else {
            MaxPort = String.valueOf(RTP_RTCP_MAX_PORT_VALUE);
            MinPort = String.valueOf(RTP_RTCP_MIN_PORT_VALUE);
            ProxyIpAdress = "";
            ProxyPortSet = "";
            ProxyEnble = false;
            mProxyCheck.setEnabled(false);
        }
    }
