                    ...
                    if (mMotionThumbItem == null || !mMotionThumbItem.hasHighResolution()) {
                        while (!mOutputDone0) {
                            ...
                            if (mMediaCodec0 == null) {
                                ...
                                int width = videoFormat.containsKey(MediaFormat.KEY_WIDTH) ? videoFormat.getInteger(MediaFormat.KEY_WIDTH) : 0;
                                int height = videoFormat.containsKey(MediaFormat.KEY_HEIGHT) ? videoFormat.getInteger(MediaFormat.KEY_HEIGHT) : 0;
                                long durationUs = videoFormat.containsKey(MediaFormat.KEY_DURATION) ? videoFormat.getLong(MediaFormat.KEY_DURATION) :0;
                                int frameRate = videoFormat.containsKey(MediaFormat.KEY_FRAME_RATE) ? videoFormat.getInteger(MediaFormat.KEY_FRAME_RATE) : 0;
                                //UNISOC added for bug 1386811, width or height can't be zero, it will cause crash in the code below,such as new CodecOutputSurface
                                if(width == 0 || height == 0){
                                    break;
                                }
                                int frameCount = (int) ((durationUs / 1000000.0f) * frameRate);
