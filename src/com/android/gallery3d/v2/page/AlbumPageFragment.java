    ...
    public void onResume() {
        ...
        if (isNextPage()) {
            if (isAddAlbumSelectItems()) {
                //UNISOC added for bug 1422584, solve the problem of title display
                int count = mSelectionManager == null ? 0 : mSelectionManager.getSelectedCount();
                if (count > 0) {
                    updateSelectItemTitle();
                } else {
                    setNavigationTitle(getString(R.string.select_images_or_videos),
                            R.drawable.action_mode_close);
                }
