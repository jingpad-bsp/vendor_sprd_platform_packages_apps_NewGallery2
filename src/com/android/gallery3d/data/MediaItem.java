...
public abstract class MediaItem extends MediaObject {
    ...
    public static final String COLUMN_IS_DRM = "is_drm";
    public static final String COLUMN_FILE_FLAG = "file_flag";

    public static final long DELTE_FILE_LARGE_SIZE = 2l * 1024l * 1024l * 1024l;
    ...
    private MotionMeta mMotionMeta;
    ...
    public boolean mIsDrmFile = false;
    public boolean mIsDrmSupportTransfer = false;
    public String mDrmFileType = "";
    /* SPRD: Drm feature end @} */

    /*
     * SPRD: Mark result of decode thumbnail, if decode failed, no need request
     * large image @{
     */
    private boolean decodeThumbnailSuccess = true;

    public void setDecodeThumbnailSuccess(boolean success) {
        decodeThumbnailSuccess = success;
    }

    public boolean getDecodeThumbnailSuccess() {
        return decodeThumbnailSuccess;
    }
    /* @} */
    ...
    public void queryJpegSize() {
        if (mFileFlag == LocalImage.IMG_TYPE_MODE_AUDIO_CAPTURE || mFileFlag == LocalImage.IMG_TYPE_MODE_HDR_AUDIO_CAPTURE
                || mFileFlag == LocalImage.IMG_TYPE_MODE_FDR_AUDIO_CAPTURE) {
            ...
            val = exif.getTagLongValue(ExifInterface.TAG_JPEG_IFD);
            if (val != null) {
                mJpegSize = val.longValue();
                return;
            }
            mJpegSize = 0;
        }
    ...
    public void bokehDonechangeFileFlag(int fileFlag) {
        mFileFlag = fileFlag;
    }
    ...
    public synchronized void setMotionMeta(MotionMeta motionMeta) {
        this.mMotionMeta = motionMeta;
    }

    public synchronized MotionMeta getMotionMeta() {
        return mMotionMeta;
    }

    //判断是否是motion photo
    public synchronized boolean isMotionPhoto() {
        int type = getMediaType();
        return type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_PHOTO
                || type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_HDR_PHOTO
                || type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_AI_PHOTO
                || type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_HDR_AI_PHOTO
                || type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_FDR_PHOTO
                || type == MediaObject.MEDIA_TYPE_IMAGE_MOTION_FDR_AI_PHOTO;
    }
}
