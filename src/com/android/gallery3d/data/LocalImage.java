    ...
    public void delete() {
        ...
        if (GalleryStorageUtil.isInInternalStorage(filePath) || Build.VERSION.SDK_INT >= ApiHelper.VERSION_CODES.Q) {
            ...
            if (getMediaType() != MediaObject.MEDIA_TYPE_IMAGE_BURST_COVER) {
                ...
                contentResolver.delete(baseUri, "(file_flag=?" + " or file_flag=?) and " + ImageColumns.DATE_TAKEN + "=?" + " and bucket_id = ? ",//Bug 1433791
                        new String[]{String.valueOf(IMG_TYPE_MODE_BURST_COVER), String.valueOf(IMG_TYPE_MODE_BURST),
                                String.valueOf(dateTaken), String.valueOf(bucketId)});
            }
        } else {
            Uri baseUri = Images.Media.EXTERNAL_CONTENT_URI;
            ContentResolver contentResolver = mApplication.getContentResolver();
